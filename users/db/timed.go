...
func (t timed) InviteUser(ctx context.Context, email, orgExternalID string, role string) (u *users.User, created bool, err error) {
	t.timeRequest(ctx, "InviteUser", func(ctx context.Context) error {
		u, created, err = t.d.InviteUser(ctx, email, orgExternalID, role)
		return err
	})
	return
}
...
func (t timed) AddUserToTeam(ctx context.Context, userID, teamID string, role string) (err error) {
	t.timeRequest(ctx, "AddUserToTeam", func(ctx context.Context) error {
		err = t.d.AddUserToTeam(ctx, userID, teamID, role)
		return err
	})
	return
}
