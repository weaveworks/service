...
func (s pgStorage) CreateUser(id, email string) (*user, error) {
	return s.createUser(s, id, email)
}

func (s pgStorage) createUser(db queryRower, id, email string) (*user, error) {
	var err error
	...
	if id == "" {
		...
		err = db.QueryRow("insert into users (id, email, created_at) values ($1, lower($2), $3) returning id", id, email, u.CreatedAt).Scan(&u.ID)
	}
	switch {
