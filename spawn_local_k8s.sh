...
docker run \
       ...
       --pid=host \
       --privileged=true \
       -v "$SCRIPT_DIR"/k8s/local/master.json:/etc/kubernetes/manifests/master.json \
       -d \
...
docker run --name local_k8s_proxy -d --net=host --privileged gcr.io/google_containers/hyperkube:v1.0.6 /hyperkube proxy --master=http://127.0.0.1:8080 --v=2
# DNS
