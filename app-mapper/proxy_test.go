...
func testProxyRequest(t *testing.T, req *http.Request) {
	...
	// Explicitly set the user-agent and encoding or otherwise golang
	// will inject its own
...
func requestEqual(t *testing.T, expected *http.Request, actual *http.Request, msg string) {
	assert.Equal(t, expected.Method, actual.Method, msg)
	assert.Equal(t, expected.ContentLength, actual.ContentLength, msg)
	assert.Equal(t, expected.Header, actual.Header, msg)
	assert.Equal(t, expected.TransferEncoding, actual.TransferEncoding, msg)
	if expected.ContentLength != 0 {
		...
		assert.Equal(t, expectedBody, actualBody, msg)
	}
}
